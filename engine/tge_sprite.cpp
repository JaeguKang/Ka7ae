...
namespace tge_sprite
{
	...
	int load(S_SPRITE_OBJECT *pObj, const char *szFileName)
	{
		...
		if (pObj->m_pSpriteBuf != NULL) { free(pObj->m_pSpriteBuf); }										//วาด็
		...
		fread(pObj->m_pSpriteBuf, _nBuffSize, 1, fp);
	...
	int save(S_SPRITE_OBJECT *pObj, const char *szFileName)
	{
		...
		fwrite(pObj->m_pSpriteBuf, sizeof(CHAR_INFO), pObj->m_header.m_cdWorkSpr.X * pObj->m_header.m_cdWorkSpr.Y, fp);
	...
	int get(S_SPRITE_OBJECT *pObj, int _xpos, int _ypos, int _width, int _height)
	{
		...
		for (_desy = 0;_desy < _height;_desy++)
		{
			...
			{
				pObj->m_pSpriteBuf[_desx + (_desy*_width)] = TGE::g_chiBuffer[_desx + ((_desy + _ypos)*SCREEN_WIDTH)];
			}
		}
		return 0;
	}
