...
namespace exam6
{
	irr::core::vector2df vPos1 = irr::core::vector2df(0, 0);
	irr::core::vector2df vPos2 = irr::core::vector2df(100, 100);
	...
	void OnKeyDown(WORD keycode)
	{
		...
		{
		case VK_LEFT:
			vPos2 += irr::core::vector2df(-2, 0);
			break;
		case VK_RIGHT:
			vPos2 += irr::core::vector2df(2, 0);
			break;
		case VK_UP:
			vPos2 += irr::core::vector2df(0, -2);
			break;
		case VK_DOWN:
			vPos2 += irr::core::vector2df(0, 2);
			break;
	...
	void OnGdiPlusRender(double fDelta, Graphics *pGrp)
	{
		...
		{
			Matrix _tmp; pGrp->GetTransform(&_tmp);
		...
		plusEngine::printf(pGrp, 0, 0, L"%lf %lf", vPos2.X, vPos2.Y);
		pGrp->ResetTransform();
	}

	void OnGdiPlusFinish()
	{
		//if (g_imgBoomber) delete g_imgBoomber;
	}
}