...
namespace exam5
{
	...
	void OnKeyDown(WORD keycode)
	{
		...
		{
		case VK_LEFT:
			vTarget += irr::core::vector2df(-2, 0);
			break;
		case VK_RIGHT:
			vTarget += irr::core::vector2df(2, 0);
			break;
		case VK_UP:
			vTarget += irr::core::vector2df(0, -2);
			break;
		case VK_DOWN:
			vTarget += irr::core::vector2df(0, 2);
			break;
	...
	void OnGdiPlusRender(double fDelta, Graphics *pGrp)
	{
...

		{
			...
			pGrp->DrawImage(g_imgBoomber,
				Rect(0, 0, g_imgBoomber->GetWidth(), g_imgBoomber->GetHeight()),
				0, 0, g_imgBoomber->GetWidth(), g_imgBoomber->GetHeight(),
				UnitPixel);
